...
fprintf(stderr, "\nallocate %u bytes", (unsigned)(size)+sizeof(int));
      ...
      {   memallocated += size;
         ...
         // if (memtraces_log != NULL)
              fprintf(/*memtraces_log*/stderr, "add %p of size %d; %d allocated (line %d, source %s)\n",
      ...
      {   int i;
          ...
          fprintf(fh, "Allocated: %d; Peak: %d\n", memallocated, mempeak);
      }

      #if 0 //def NDEBUG
      #   define realloc(p, size) realloc2(p, size)
      #   define malloc(size) realloc2(NULL, size)
      #   define calloc(nmemb,size) ( pmemtemp=(size), memset(realloc2(NULL, pmemtemp), 0, pmemtemp) )
      ...
      #   define calloc(nmemb,size) ( pmemtemp=(size), memset(memtraces_add(realloc2( \
